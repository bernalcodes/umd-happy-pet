...
const Step1 = ({
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      <div className="flex w-full flex-col items-start  gap-[0.75rem] lg:gap-[1.3125rem]">
        <div className="flex w-full flex-row items-start gap-[0.75rem]">
          <div className="flex w-1/2 flex-grow flex-col gap-1 ">
            <input
              className={`input-style w-full ${
                errors.name ? "border-[#ff9494]" : ""
              }`}
        ...
        <div className="flex w-full flex-col gap-1">
          <input
            type="number"
            className={`input-style w-full ${
              errors.phoneNumber ? "border-[#ff9494]" : ""
            }`}
      ...
      <div className="flex w-full flex-col items-start  gap-6 lg:gap-5">
        <div className="flex w-full flex-col gap-4">
          <span className=" text-sm font-light text-happy-color-text lg:text-base">
            Select the type of user you are
          </span>
          <ul className="flex w-full flex-row justify-around gap-2 rounded-2xl border-2 border-happy-color-primary p-1">
            {typeUsers.map((type) => (
              <li
                key={`type-${type}`}
                ...
                } relative h-full w-full cursor-pointer rounded-2xl p-[0.625rem] text-center text-sm font-medium transition-colors lg:text-base`}
              >
          ...
          <label htmlFor="input-address" className="cursor-pointer">
            <div className="p-[11px]">
              {typeUserSelected === typeUsers[0] ? (
                <svg
                  className="h-[1.875rem] w-auto stroke-happy-color-primary"
                  width="22"
                  height="22"
                  viewBox="0 0 22 22"
                  fill="none"
          ...
          <div className="flex w-full flex-col gap-1">
            {typeUserSelected === typeUsers[0] && (
              <>
                ...
                />
                {errors.address && (
                  <FormErrorText message={errors.address?.message} />
                )}
              </>
            )}
            {typeUserSelected === typeUsers[1] && (
              <>
                <input
                  className={`input-style w-full ${
                    errors.cardNumber ? "border-[#ff9494]" : ""
                  }`}
                ...
                />
                {errors.cardNumber && (
                  <FormErrorText message={errors.cardNumber?.message} />
                )}
              </>
            )}
          </div>
